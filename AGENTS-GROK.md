...
- **`vite.config.ts` + `tsconfig.json`** — preconfigured (preview port
  ...
  must, but keep the port, the build/preview-gated nitro plugin (see §"Build &
...
**Share-card tags are platform chrome.** Do not put `og:*` or `twitter:card` in `__root.tsx` — the PWA injector (`scripts/grok-pwa-shared.mjs`) overwrites them on every HTML response. Title defaults to the host slug. If the display name is not the slug, or the app is a game, create `src/lib/og/site.json` (`{ "title": "My App", "type": "x:game", "card": "custom" }`). Custom art is `public/og.jpg` **and** `"card": "custom"` in that JSON. Bake infers custom from the file if the flag is missing, but brand-check still requires the field. Games also ship `public/x-banner.jpg` (50:11, 1200×264) — see the **`og` skill**. Live preview emits the same tags (via `X-Forwarded-Host`). Identity is baked at `vite build` so published Nitro can inject without reading the workspace filesystem.
...
Vercel runs. Run both **in the background, in parallel**, as soon as the source
is stable, and do the browser QA against the dev server while they run — don't
serialize them behind it.

A passing build isn't enough: serve the built output with `npm run preview` —
it serves the built `.vercel/output` on **loopback `127.0.0.1:8081`** (never
the `:8080` live preview) — then smoke it with the dev run's JSON verdict as
baseline (second command in the smoke snippet below).
...
recreate them. The vite config binds the dev port, pins `vite preview` to
loopback `127.0.0.1:8081` (so the built output can never be picked up as the
user's live preview), and gates `nitro({ preset: "vercel" })` on
`command === "build" || isPreview` so it never runs in dev (left on in dev,
nitro opens a second dev-server port, which breaks the single-port 8080 live
preview) but still serves the built output under `vite preview`. If you edit
it, preserve both port contracts (dev `0.0.0.0:8080`, preview
`127.0.0.1:8081`), the build/preview-gated nitro plugin (including its
`serverDir: "./server"` option — without it the deployed app loses the Home
Screen install page), and `grokPwaPlugin()`.

```bash
npm run dev         # 0.0.0.0:8080 — run in background when ready; leave it up
npm run build       # background it, in parallel with typecheck + browser QA
npm run typecheck
npm run preview     # built output on 127.0.0.1:8081 — QA only, never the live preview
```
...
# ONE run audits desktop (1280×800) AND mobile (390×844): two PNGs (mobile gets
# a -mobile suffix) + a JSON verdict (per-viewport console/page errors, body
# text, horizontal overflow) on stdout and next to the PNG (.json).
# Writes under /workspace/screenshots/ by default (never /tmp).
node scripts/browser-smoke.mjs http://127.0.0.1:8080/

# Built-output check (npm run preview → 127.0.0.1:8081): reuse the dev verdict
# as baseline — the JSON reports divergesFromBaseline so you only re-read
# screenshots when it flags.
...
| Game / interactive | voxels, clicker, puzzle, kart, flight | Canvas/WebGL/DOM — self-contained single-player (or + bots). For 2-8 player co-op/casual realtime, use the **`multiplayer-p2p` skill** (WebRTC mesh; not for competitive/cheat-sensitive play). For WASD / vehicle / flight, open **`.grok/skills/controls/SKILL.md`** before writing movement (inverted A/D is a common ship-blocker) and use **`building-games`** for loop/3D. Games also ship a custom `public/og.jpg` share card, `"type": "x:game"` in `src/lib/og/site.json`, and `public/x-banner.jpg` — open **`.grok/skills/og/SKILL.md`** (placeholder card not acceptable; X uses injector-emitted `og:type=x:game` for game-card presentation) |
...
- **2D game sprites / animation sheets** (characters, walk cycles, attacks,
  ...
  **`game-character-consistency`** when relevant) for QC and defaults. When gen
  ...
  missing tools. **Exception — abstract/geometric games** (tetris, snake, pong,
  breakout, and the like) are correctly rendered procedurally: generated sprite
  sheets there are a quality regression, not an upgrade — do **not** invoke
  image generation for them even when gen tools are listed.
...
- **Brand assets — a custom share card is the default**: open the **`og`**
  ...
  also set `"type": "x:game"` in `src/lib/og/site.json`** (X presents the unfurl as a
  game card) **and generate `public/x-banner.jpg`** (50:11 / 1200×264) — see the og skill;
...
- **Follow-up turns (multi-turn continuity):** when the preview is already
  ...
  user's preview mid-session. After edits, re-run the smoke pass; re-read the
  screenshots only when the edit touched visible UI.
...
2. Assign **non-overlapping surfaces** (e.g. page A vs page B, or data layer vs
   ...
   needs the name and palette, and only writes `public/` + `src/lib/og/site.json`.
...
3. **Playwright helper (preinstalled)** — one run loads desktop + mobile,
...
landing page usually needs nothing more than that pass. For a
...
- **Production build passes** and the **built output renders** (not just
  `npm run dev` / not just a green `npm run build`) — smoke-checked on
  `:8081` against the dev baseline. Build/typecheck run **in the background,
  overlapped with browser QA**.
- **Typecheck passes** (`npm run typecheck`).
- **Required:** one real browser render check on dev **and** on the production
  build; both show visible content with a clean console. Inspect both viewport
  PNGs in one batched read every time; re-read built-output screenshots only
  when the JSON verdict flags a failure or divergence.
...
- **Share cards** come from the PWA injector, not root meta. Create
  `src/lib/og/site.json` when the title is not the host slug or the app is a
  game (`"type": "x:game"`), plus `public/og.jpg` **and** `"card": "custom"`
  for any app with a face — per **`.grok/skills/og/SKILL.md`**. Games also
  ship `public/x-banner.jpg` (50:11). `browser-smoke.mjs` prints a
  `BRAND WARNING` for missing card / missing `"card": "custom"` / missing
  `type` / missing `x-banner.jpg` on canvas apps; treat it as **not done**,
...
overlap:   background npm run build + npm run typecheck in parallel; browser-QA meanwhile
verify:    ONE browser-smoke.mjs run = desktop + mobile + JSON verdict; inspect both dev
           PNGs every time (one batched read) — not curl-only
built:     npm run preview → 127.0.0.1:8081 (never :8080) → smoke with --baseline dev JSON;
           source changed after the build? rebuild + restart preview; re-read built PNGs
           only on failure/divergence
controls:  WASD/vehicle/flight → .grok/skills/controls/SKILL.md; A=left self-test
og:        injector owns og:* / twitter:card (scripts/grok-pwa-shared.mjs). create src/lib/og/site.json only if title ≠ slug or game type; public/og.jpg + "card":"custom" for custom cards; games also public/x-banner.jpg (50:11). do not put og tags in __root.tsx. brand assets in a parallel `task` by default, kicked off right after scaffold, wait_tasks before final verify
...
sprites:   doctrine → game-asset-core+specialist; pipeline → generate2dsprite (#FF00FF); maps → generate2dmap; dense motion → video2dsprite
           NOT for geometric games (tetris/snake/pong/breakout) — those stay procedural
