---
id: nodez-graphify-tech-research
...
Research into the three technical pillars Nodez wants from Graphify: how the graph is **stored**, how it is **served to agents over MCP**, and how it is **visualized** — followed by a prioritized view of what is doable in Nodez now versus what needs the [[Tauri Desktop Shell]].
...
Nodez already has the vocabulary for this in [[Unified Knowledge System]] (node types, edge types, `extracted`/`inferred`/`manual` provenance). This note is about the machinery underneath.

## 1. Storage layer

### Extraction

How the raw graph gets built. In priority order for a Nodez-shaped vault + repo:

- **Markdown / YAML (already doable today).** Wikilinks, frontmatter, tags, and headings are parsed with a plain Markdown AST. This is the vault index Nodez already computes in the prototype. Provenance = `extracted`.
...
Four realistic options, roughly in order of when Nodez should adopt them:

- **In-memory + JSON snapshot (now).** Build the graph in memory (JS objects / `Map`s), persist a `graph.json` under `.nodez/`. Zero dependencies, works in the browser prototype, diff-able in git, matches "rebuild from files, no database." Good to a few thousand nodes. This is the right Phase 0/1 store.
...
- **Embedded property-graph DB (the "real graph" option).** **KuzuDB** remains a candidate for Nodez if graph queries outgrow JSON/SQLite. The verified Graphify open-source package currently treats `graphify-out/graph.json` as the primary downstream artifact, with optional exports to systems such as GraphML, Obsidian, Neo4j, and FalkorDB.
- **Neo4j / hosted graph DB — explicitly avoid.** Server-based, conflicts with the local-first, no-proprietary-service decisions in [[Decision Log]]. Only revisit if Nodez ever grows a multi-user backend.
...
- Keep the whole store under `.nodez/` so it is app metadata, not note content.
...
- **Implementation:** an official MCP SDK exists for **TypeScript** and **Python** (and community Rust). Because Nodez's index logic will already be in TS (prototype) and later Rust (Tauri), a TS MCP server that reads the same `graph.json`/SQLite store is the lowest-friction start; a Rust sidecar is the native end-state.
...
- Expose notes and source files as MCP resources (`nodez://note/<id>`, `nodez://file/<path>`) so the agent can read the specific artifact a query points to. This keeps `AGENTS.md`'s "query the graph, then read the files it points to" loop tight.
...
- **Canvas 2D default (`GraphifyNetwork.tsx`).** Dependency-free Barnes-Hut force simulation on canvas. Default engine for Nodez; Graphify-like interaction (degree sizing, community colors, path dimming, click inspect) without shipping `vis-network`.
...
- **`vis-network` (historical Graphify exporter default; superseded in Nodez).** Graphify's open-source HTML exporter uses `vis-network@9.1.6`. Nodez no longer depends on it.
- **`react-force-graph` 2D (not the Nodez default).** React wrapper over `d3-force` for custom canvas work; Nodez uses the in-house canvas path instead.
...
## What's doable in Nodez — prioritized
...
- Formalize the **node/edge schema** (with provenance/confidence) as the in-memory model + a `graph.json` snapshot in `.nodez/`. (Backlog: "Node and edge schema for unified graph", "Edge provenance".)
...
- This is what turns Nodez from "a nice editor" into the Dakila unified system, so it is worth pulling earlier than its Phase 4 slot if agent workflows are a priority.

## Recommended concrete stack

- Extraction: Markdown AST (now) + tree-sitter WASM (soon) + tree-sitter native / LSP (Tauri).
- Store: `graph.json` in `.nodez/` now → SQLite (`sql.js` browser / `rusqlite` native) → KuzuDB only if needed **(verify vs Graphify)**.
...
- Graphify's official local artifact is `graphify-out/graph.json`; Nodez should stay compatible with this format while keeping its own richer app model.
- Graphify's documented MCP server exposes `query_graph`, `get_node`, `get_neighbors`, `shortest_path`, `get_community`, `god_nodes`, `graph_stats`, `list_prs`, `get_pr_impact`, and `triage_prs`, plus read-only graph resources. Nodez now implements the core local graph tool subset.
